...
import {
  checkCounts, checkMarkerLeaks, checkReferentialIntegrity, checkSnapshotLineEndings, renderManifest,
} from "./manifest.mjs";
...
// Guard against a CRLF snapshot, also before clearOwned() runs. A checkout
// on Windows with the common core.autocrlf=true default (the upstream repo
// carries no .gitattributes to override it) silently rewrites every "\n" to
...
const crlfSample = CRLF_SAMPLE_FILES
...
if (crlfProblems.length > 0) {
  ...
  for (const p of crlfProblems) console.error(`  ${p}`);
  process.exit(2);
}
...
/** The first line's heading text (leading `#`s and whitespace stripped), for the templates index. */
...
const templateIndexEntries = [];
for (const { rel, full } of walkMd(templatesSrcDir)) {
  const raw = readFileSync(full, "utf8");
  const outPath = `templates/${rel}`;
  recordBashSites(raw, outPath);
  const body = rewriteBody(raw, DEST.DOC, outPath);
  emit(outPath, body);
  templatesCount++;
  // rel, not outPath: the index's own rows are relative to templates/
  // itself, same as roles/_index.md's rows are bare role names rather than
  // "roles/<name>.md". Title comes from the SHIPPED body (post-rewrite),
  // since that is what a reader following this index actually sees.
  templateIndexEntries.push({ rel, title: firstHeading(body) });
}

// Generated AFTER the loop above, same mechanism as roles/_index.md: this
// keeps templatesCount at 40 (the index is never counted — see
// EXPECTED_COUNTS' doc comment) while disk carries 41 files. Do not add
// this index to templatesCount.
const templateIndexLines = [
  "# Templates index", "",
...
for (const t of templateIndexEntries) {
  templateIndexLines.push(`- \`${t.rel}\` — ${t.title}`);
}
emit("templates/_index.md", templateIndexLines.join("\n") + "\n");

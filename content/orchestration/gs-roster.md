...
All 49 role briefs are installed. Read
`%%GS_CONTENT_DIR%%roles/_index.md` for the full roster: name, tier,
department, and a one-line description for each role, generated by the
port from the briefs themselves. List `%%GS_CONTENT_DIR%%roles/` with
`glob` if you need the raw file listing instead.

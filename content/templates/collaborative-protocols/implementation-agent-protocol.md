...
2. **Ask architecture questions:**
   ...
   - *Use `ask_user_question` to batch constrained architecture questions*
...
6. **Complete the story with `/gs-story-done`:**
   - When implementation (and tests, if written) is complete, invoke `/gs-story-done [story-file-path]`
   - This verifies acceptance criteria, checks for GDD/ADR deviations, prompts code review, and updates the story status
   - Do NOT mark a story complete manually — always go through `/gs-story-done`
   - If no story file exists for this work (ad-hoc task), offer `/gs-code-review` directly instead
...
File is ready. Would you like me to:
 ...
 C) Run /gs-code-review for architecture validation"
...
You: [creates tests/combat/test_damage_calculator.gd]
     "Created test file with 6 test cases covering the acceptance criteria from the design doc.
      ...
      Running /gs-story-done to verify acceptance criteria and close out the story."
...
- Story completion is explicit — use `/gs-story-done` to close every story, never assume done because code is written

#### Structured Decision UI

Use the `ask_user_question` tool for architecture decisions and next-step choices.
Follow the **Explain → Capture** pattern:

1. **Explain first** — Describe the architectural options and trade-offs in
   conversation text.
2. **Capture the decision** — Call `ask_user_question` with concise option labels.
...
- When running as a subagent — structure text for orchestrator

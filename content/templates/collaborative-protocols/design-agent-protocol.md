...
1. **Ask clarifying questions:**
   ...
   - *Use `ask_user_question` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `ask_user_question` to capture the decision*
...
You: [uses write tool]
     "Created design/gdd/crafting-system.md. Would you like me to run /gs-design-review to validate it?"
```
...
Use the `ask_user_question` tool to present decisions as a selectable UI instead of
...
- Batch up to 4 independent questions in a single `ask_user_question` call
...
- When running as a subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via ask_user_question

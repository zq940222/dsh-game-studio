...
- [ ] Fill in AGENTS.md technology stack based on engine choice (`/gs-setup-engine`)
- [ ] Create game pillars document (`/gs-design-review` to validate)
- [ ] **Prototype core idea** (`/gs-prototype [core-mechanic]`) — before writing GDDs, validate the concept is worth designing
- [ ] If prototype PROCEEDS: Decompose concept into systems (`/gs-map-systems`)
- [ ] Design each system (`/gs-design-system [system-name]`) — use prototype learnings in Tuning Knobs and Formulas sections
- [ ] Build vertical slice in Pre-Production (`/gs-vertical-slice`) — validate full game loop before committing to Production
- [ ] Validate core loop with playtest (`/gs-playtest-report`)
- [ ] Plan first milestone (`/gs-sprint-plan new`)

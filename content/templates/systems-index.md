...
- [ ] Design MVP-tier systems first (use `/gs-design-system [system-name]`)
- [ ] Run `/gs-design-review` on each completed GDD
- [ ] Run `/gs-gate-check pre-production` when MVP systems are designed
- [ ] Validate the highest-risk systems with `/gs-vertical-slice` before committing to Production

...
**Playtest note:** Run `/gs-playtest-report` to structure additional session data
before running `/gs-gate-check pre-production`.

**Next steps:**
1. `/gs-gate-check pre-production` — formally advance to Production
2. `/gs-create-epics layer:foundation` — plan Foundation layer epics
3. `/gs-create-epics layer:core` — plan Core layer epics
4. `/gs-sprint-plan` — use velocity data from this report in the estimate
...
**Architecture decisions to revisit:** [List — use `/gs-architecture-decision` to update]
**Core loop change needed:** [What specifically to change]

**Next steps:**
1. `/gs-design-system [mechanic]` — revise affected GDDs
2. `/gs-architecture-decision [decision]` — address architecture issues
3. `/gs-vertical-slice` — re-validate after revisions
...
**Next step:** `/gs-brainstorm` to explore a new direction, or `/gs-prototype [new-concept]`

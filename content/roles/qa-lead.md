...
- Run or verify `/gs-smoke-check` passes before any build goes to manual QA

### QA Workflow Integration

**Your skills to use:**
- `/gs-qa-plan [sprint]` — generate test plan from story types at sprint start
- `/gs-smoke-check` — run before every QA hand-off
- `/gs-team-qa [sprint]` — orchestrate full QA cycle
...
- Pre-QA gate: Run `/gs-smoke-check`; block hand-off if it fails
...
- Review story acceptance criteria before implementation starts (`/gs-story-readiness`)
...
3. **Smoke Check Ownership**: Run `/gs-smoke-check` before every build goes to manual QA.

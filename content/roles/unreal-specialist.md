...
- delegate to `ue-gas-specialist` (the child reads `roles/ue-gas-specialist.md` itself) — Gameplay Ability System, effects, attributes, tags
- delegate to `ue-blueprint-specialist` (the child reads `roles/ue-blueprint-specialist.md` itself) — Blueprint architecture, BP/C++ boundary, optimization
- delegate to `ue-replication-specialist` (the child reads `roles/ue-replication-specialist.md` itself) — Property replication, RPCs, prediction, relevancy
- delegate to `ue-umg-specialist` (the child reads `roles/ue-umg-specialist.md` itself) — UMG, CommonUI, widget hierarchy, data binding

---
name: gs-create-stories
description: Break a single epic into implementable story files. Reads the epic, its GDD, governing ADRs, and control manifest. Each story embeds its GDD requirement TR-ID, ADR guidance, acceptance criteria, story type, and test evidence path. Run after /gs-create-epics for each epic.
...
**Previous step:** `/gs-create-epics [system]`
**Next step after stories exist:** `/gs-story-readiness [story-path]` then `/gs-dev-story [story-path]`
...
in this skill — apply the check pattern from `../../handbook/director-gates.md`
before every gate invocation.

- `/gs-create-stories [epic-slug]` — e.g. `/gs-create-stories combat`
- `/gs-create-stories production/epics/combat/EPIC.md` — full path also accepted
...
> Check the filename in the epic's Governing ADRs list, or run `/gs-architecture-decision`
...
The type determines what test evidence is required before `/gs-story-done` can close the story.
...
- **Governing ADR**: which ADR governs how to implement this?
  ...
  - `Status: Proposed` → set story `Status: Blocked` with note: "BLOCKED: ADR-NNNN is Proposed — run `/gs-architecture-decision` to advance it"
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via a subagent using gate **QL-STORY-READY** (`../../handbook/director-gates.md`).
...
- If "Skip": leave `## QA Test Cases` with a placeholder: `*Test cases not yet defined — run /gs-qa-plan to generate them.*`
...
> **Last Updated**: [set by /gs-dev-story when implementation begins]
...
Use `ask_user_question` to close with context-aware next steps:

Check:
- Are there other epics in `production/epics/` without stories yet? List them.
- Is this the last epic? If so, include `/gs-sprint-plan` as an option.

Widget:
- Prompt: "[N] stories written to `production/epics/[epic-slug]/`. What next?"
- Options (include all that apply):
  - `[A] Start implementing — run /gs-story-readiness [first-story-path]` (Recommended)
  - `[B] Create stories for [next-epic-slug] — run /gs-create-stories [slug]` (only if other epics have no stories yet)
  - `[C] Plan the sprint — run /gs-sprint-plan new` (only if all epics have stories)
...
- **Verdict: COMPLETE** — [N] stories written to `production/epics/[epic-slug]/`. Run `/gs-story-readiness` → `/gs-dev-story` to begin implementation.

...
> "Usage: `/gs-team-combat [combat feature description]` — Provide a description of the combat feature to design and implement (e.g., `melee parry system`, `ranged weapon spread`)."
...
- **engine specialist** (primary) — Validate architecture and implementation patterns are idiomatic for the engine (read from `../../handbook/technical-preferences.md` Engine Specialists section)
- **qa-tester** — Write test cases and validate the implementation

## How to Delegate

Use the subagent tool to spawn each team member as a subagent:
- delegate to `game-designer` (the child reads `roles/game-designer.md` itself) — Design the mechanic, define formulas and edge cases
- delegate to `gameplay-programmer` (the child reads `roles/gameplay-programmer.md` itself) — Implement the core gameplay code
- delegate to `ai-programmer` (the child reads `roles/ai-programmer.md` itself) — Implement NPC/enemy AI behavior
- delegate to `technical-artist` (the child reads `roles/technical-artist.md` itself) — Create VFX, shader effects, visual feedback
- delegate to `sound-designer` (the child reads `roles/sound-designer.md` itself) — Define audio events, impact sounds, ambient audio
- a subagent for `[primary engine specialist]` (the child reads its own brief under `roles/`) — Engine idiom validation for architecture and implementation
- delegate to `qa-tester` (the child reads `roles/qa-tester.md` itself) — Write test cases and validate implementation
...
delegated to sub-agents spawned via a subagent. Each sub-agent enforces the

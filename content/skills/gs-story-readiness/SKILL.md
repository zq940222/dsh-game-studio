...
See `../../handbook/director-gates.md` for the full check pattern and mode definitions.

---

## 1. Parse Arguments

**Scope:** `$ARGUMENTS[0]` (blank = ask user via ask_user_question)

- **Specific path** (e.g., `/gs-story-readiness production/epics/combat/story-001-basic-attack.md`):
...
If no argument is given, use `ask_user_question`:
...
- [ ] **TR-ID is valid and active**: If the story contains a `TR-[system]-NNN`
  ...
  - If the ID does not exist in the registry → NEEDS WORK: ID was not registered
    (story may predate registry, or registry needs an `/gs-architecture-review` run).
...
Resolve these before the sprint begins or replan with `/gs-sprint-plan update`.
```
...
`/gs-create-stories`) handles writing.

**Redirect rules:**
- If a story file does not exist at all: "This story file is missing entirely.
  Run `/gs-create-epics [layer]` then `/gs-create-stories [epic-slug]` to generate stories from the GDD and ADR."
- If a story has no GDD reference and the work appears small: "This story has
  ...
  `/gs-quick-design [description]` to create a Quick Design Spec, then reference
...
Run `/gs-story-readiness [path]` to validate before starting.
```
...
Spawn `qa-lead` via a subagent using gate **QL-STORY-READY** (`../../handbook/director-gates.md`).
...
- Run `/gs-dev-story [story-path]` to begin implementation once the story is READY
- Run `/gs-story-readiness sprint` to check all stories in the current sprint at once
- Run `/gs-create-stories [epic-slug]` if a story file is missing entirely

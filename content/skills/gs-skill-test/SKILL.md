...
|------|---------|---------|------------|
| `static` | `/gs-skill-test static [name\|all]` | Structural linter — 7 compliance checks per skill | Low (~1k/skill) |
| `spec` | `/gs-skill-test spec [name]` | Behavioral verifier — evaluates assertions in test spec | Medium (~5k/skill) |
| `category` | `/gs-skill-test category [name\|all]` | Category rubric — checks skill against its category-specific metrics | Low (~2k/skill) |
| `audit` | `/gs-skill-test audit` | Coverage report — skills, agent specs, last test dates | Low (~3k total) |
...
- `static all` → run 7 structural checks on all skills (glob `../../skills/gs-*/SKILL.md`)
...
- A final section mentioning another skill (e.g., `/gs-story-done`, `/gs-gate-check`)
...
glob `../../skills/gs-*/SKILL.md` to get the complete list of skills.
...
Offer: "Would you like to run `/gs-skill-test static all` to check structural
compliance across all skills? `/gs-skill-test category all` to run category rubric
checks? Or `/gs-skill-test spec [name]` to run a specific behavioral test?"
...
- After `static [name]`: "Run `/gs-skill-test spec [name]` to validate behavioral
  correctness if a test spec exists."
- After `static all` with failures: "Address NON-COMPLIANT skills first. Run
  `/gs-skill-test static [name]` individually for detailed remediation guidance."
- After `spec [name]` PASS: "Update `CCGS Skill Testing Framework/catalog.yaml` to record this
  pass date. Consider running `/gs-skill-test audit` to find the next spec gap."

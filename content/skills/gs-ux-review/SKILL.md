...
the `/gs-team-ui` pipeline.

**Run this skill:**
- After completing a UX spec with `/gs-ux-design`
...
- **Specific file path** (e.g., `/gs-ux-review design/ux/inventory.md`): validate
  that one document
...
1. **Input & Platform config**: Read `../../handbook/technical-preferences.md` and
...
[For APPROVED]: This spec is ready for handoff to `/gs-team-ui` Phase 2
(Visual Design).
...
Recommend returning to `/gs-ux-design` to rework [sections].
```
...
- For **APPROVED**: suggest running `/gs-team-ui` to begin implementation coordination
- For **NEEDS REVISION**: offer to help fix specific gaps ("Would you like me to
  ...
  instruction
- For **MAJOR REVISION NEEDED**: suggest returning to `/gs-ux-design` with the

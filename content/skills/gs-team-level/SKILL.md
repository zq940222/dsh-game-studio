...
- delegate to `narrative-director` (the child reads `roles/narrative-director.md` itself) — Narrative purpose, characters, emotional arc
- delegate to `world-builder` (the child reads `roles/world-builder.md` itself) — Lore context, environmental storytelling, world rules
- delegate to `level-designer` (the child reads `roles/level-designer.md` itself) — Spatial layout, pacing, encounters, navigation
- delegate to `systems-designer` (the child reads `roles/systems-designer.md` itself) — Enemy compositions, loot tables, difficulty balance
- delegate to `art-director` (the child reads `roles/art-director.md` itself) — Visual theme, color palette, lighting, asset requirements
- delegate to `accessibility-specialist` (the child reads `roles/accessibility-specialist.md` itself) — Navigation clarity, colorblind safety, cognitive load
- delegate to `qa-tester` (the child reads `roles/qa-tester.md` itself) — Test cases, boundary testing, playtest checklist
...
**Gate**: Use `ask_user_question` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
**Gate**: Use `ask_user_question` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `ask_user_question` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `ask_user_question` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:

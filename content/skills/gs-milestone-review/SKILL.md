...
Before generating the Go/No-Go recommendation, spawn `producer` via a subagent using gate **PR-MILESTONE** (`../../handbook/director-gates.md`).
...
If OFF TRACK, use `ask_user_question` before generating the recommendation:
...
If AT RISK, use `ask_user_question`:
...
- Run `/gs-gate-check` for a formal phase gate verdict if this milestone marks a development phase boundary.
- Run `/gs-sprint-plan` to adjust the next sprint based on the scope recommendations above.

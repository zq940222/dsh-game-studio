...
- delegate to `qa-lead` (the child reads `roles/qa-lead.md` itself) — Strategy, planning, classification, sign-off
- delegate to `qa-tester` (the child reads `roles/qa-tester.md` itself) — Test case writing and bug report writing
...
Spawn `qa-lead` via a subagent to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use `ask_user_question`:
...
options:
  ...
  - "Smoke check failed — fix issues and re-run /gs-team-qa"
  - "Cancel — resolve blockers first"
```

If smoke check **FAIL**: do not proceed to Phase 3. Surface the failures from the smoke check report and stop. The user must fix them, re-run `/gs-smoke-check sprint`, and then re-run `/gs-team-qa`.
If smoke check **UNKNOWN**: surface a warning — "No smoke check report found. Recommend running `/gs-smoke-check sprint` before QA. Proceeding with caution."
...
Spawn `qa-tester` via a subagent for each story (run in parallel where possible), providing:
- The story file path
...
Use `ask_user_question` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `ask_user_question` for each:
...
After each FAIL result: use `ask_user_question` to collect the failure description, then spawn `qa-tester` via a subagent to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` via a subagent to produce the sign-off report using all results from Phases 4–6.
...
- APPROVED: "Build is ready for the next phase. Run `/gs-gate-check` to validate advancement."
- APPROVED WITH CONDITIONS: "Resolve conditions before advancing. S3/S4 bugs may be deferred to polish."
- NOT APPROVED: "Resolve S1/S2 bugs and re-run `/gs-team-qa` or targeted manual QA before advancing."
...
Usually executed by the `qa-lead` role. Load `gs-roster` for the delegation protocol.

...
1. **Read engine config**:
   - Read `../../handbook/technical-preferences.md` and extract the `Engine:` value.
   - If engine is not configured (`[TO BE CONFIGURED]`), stop:
     "Engine not configured. Run `/gs-setup-engine` first, then re-run `/gs-test-setup`."
...
3. **Report findings**:
   ...
   - If everything already exists AND `force` argument was not passed:
     "Test infrastructure appears to be in place. Re-run with `/gs-test-setup force`
...
tests/
  ...
  smoke/          # Critical path test list for /gs-smoke-check gate
...
```gdscript
# GdUnit4 test runner — invoked by CI and /gs-smoke-check
  ...
  test:
    ...
    steps:
      ...
      - name: Run GdUnit4 Tests
        ...
        with:
          godot-version: '[VERSION FROM ../../engines/godot/VERSION.md]'
          paths: |
            tests/unit
            tests/integration
...
**Run via**: `/gs-smoke-check` (which reads this file)
...
3. Run `/gs-qa-plan sprint` before your first sprint to classify stories and set
   test evidence requirements
4. `/gs-smoke-check` before every QA hand-off

Gate note: /gs-gate-check Technical Setup → Pre-Production now requires:
- tests/ directory with unit/ and integration/ subdirectories
- .github/workflows/tests.yml
- At least one example test file
Run /gs-test-setup and write one example test before advancing.
...
- **Engine detection is non-negotiable** — if the engine is not configured,
  stop and redirect to `/gs-setup-engine`. Do not guess.

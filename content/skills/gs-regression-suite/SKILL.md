...
- Before a release gate (`/gs-gate-check polish` requires regression suite exists)
...
**Modes:**
- `/gs-regression-suite update` — scan new bug fixes this sprint and check
  for regression test presence; add new tests to the suite manifest
- `/gs-regression-suite audit` — full audit of all GDD critical paths vs.
  existing test coverage; flag paths with no regression test
- `/gs-regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, use `ask_user_question`:
...
- For each HIGH priority gap: "Consider creating the missing regression test
  before the next sprint. Run `/gs-test-helpers` to scaffold the test file."
...
- If coverage drift detected: "Regression suite may be drifting. Consider
  running `/gs-regression-suite audit` at the next sprint boundary."

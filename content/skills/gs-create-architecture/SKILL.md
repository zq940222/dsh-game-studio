...
**Distinct from `/gs-architecture-decision`**: ADRs record individual point decisions.
...
3. `../../handbook/technical-preferences.md` — naming conventions, performance budgets,
...
⚠️  [ClassName.method()] — Godot 4.6 (post-cutoff, HIGH risk)
    Verified against: ../../engines/godot/modules/[domain].md
...
- `/gs-architecture-decision [title]` → covers: TR-[id], TR-[id]

**Core Layer:**
- `/gs-architecture-decision [title]` → covers: TR-[id]
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `ask_user_question`:
...
Apply gate **TD-ARCHITECTURE** (`../../handbook/director-gates.md`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Spawn `lead-programmer` via a subagent using gate LP-FEASIBILITY (`../../handbook/director-gates.md`):**
...
Use `ask_user_question` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `ask_user_question`:
...
**1. `/gs-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence: what it defines and what it unblocks.]

**2. `/gs-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence.]

**3. `/gs-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence.]
...
> **Required before `/gs-gate-check [stage]`:**
> - [ ] Accept ADRs: [list Proposed ADR IDs that must be Accepted]
> - [ ] Write ADRs: [list ADR IDs that must still be written]
> - [ ] Run `/gs-test-setup` — scaffolds `tests/unit/`, `tests/integration/`, CI workflow, and an example test file
> - [ ] Run `/gs-ux-design` — creates `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md`
>
> Run `/gs-gate-check [stage]` when all boxes are checked.

If nothing is blocking, write instead:
> No blockers — run `/gs-gate-check [stage]` now.
...
5. **Use `ask_user_question` for write approvals** — plain text "May I?" is not
...
- Run `/gs-architecture-decision [title]` for each required ADR listed in Phase 6 — Foundation layer ADRs first
- Run `/gs-architecture-review` — bootstraps the Requirements Traceability Matrix and TR registry from the ADRs just written. Required before the Pre-Production gate.
- Run `/gs-test-setup` to scaffold `tests/unit/`, `tests/integration/`, CI workflow, and an example test (required for gate-check)
- Run `/gs-ux-design` to initialize `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md` (required for gate-check)
- Run `/gs-create-control-manifest` once the required ADRs are written to produce the layer rules manifest
- Run `/gs-gate-check pre-production` when all required ADRs, `/gs-test-setup`, and `/gs-ux-design` are complete

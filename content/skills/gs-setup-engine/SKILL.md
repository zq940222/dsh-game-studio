---
name: gs-setup-engine
description: Configure the project's game engine and version. Pins the engine in AGENTS.md, detects knowledge gaps, and populates engine reference docs via web_search when the version is beyond the LLM's training data.
...
- **Full spec**: `/gs-setup-engine godot 4.6` — engine and version provided
- **Engine only**: `/gs-setup-engine unity` — engine provided, version will be looked up
- **No args**: `/gs-setup-engine` — fully guided mode (engine recommendation + version)
- **Refresh**: `/gs-setup-engine refresh` — update reference docs (see Section 10)
- **Upgrade**: `/gs-setup-engine upgrade [old-version] [new-version]` — migrate to a new engine version (see Section 11)
...
- Read `design/gdd/game-concept.md` if it exists — extract genre, scope, platform
  targets, art style, team size, and any engine recommendation from `/gs-brainstorm`
- If no concept exists, inform the user:
  > "No game concept found. Consider running `/gs-brainstorm` first to discover what
...
**Question 2 — Target platform** (ask this second, always, via `ask_user_question` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `ask_user_question` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `ask_user_question` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
After updating AGENTS.md, create or update `../../handbook/technical-preferences.md` with
...
### Remaining Sections
- **Performance Budgets**: Use `ask_user_question`:
...
Create a minimal `../../engines/<engine>/VERSION.md`:
...
Run `/gs-setup-engine refresh` to populate full reference docs at any time.
```
...
Ask: "May I create the engine reference docs under `../../engines/<engine>/`?"
...
1. Read `../../engines/<engine>/VERSION.md`
...
1. Read the existing `../../engines/<engine>/VERSION.md` to get
...
If invoked as `/gs-setup-engine upgrade [old-version] [new-version]`:

### Step 1 — Read Current Version State

Read `../../engines/<engine>/VERSION.md` to confirm the current pinned
...
Use web_search and web_fetch to locate the official migration guide between
...
1. Update `../../engines/<engine>/VERSION.md`:
...
2. Run /gs-setup-engine refresh after upgrading the actual engine binary to
   verify no new deprecations were missed
3. Run /gs-architecture-review — the engine upgrade may invalidate ADRs that
   reference specific APIs or engine capabilities
4. If any ADRs are invalidated, run /gs-propagate-design-change to update
   downstream stories
```
...
Next Steps:
1. Review ../../engines/<engine>/VERSION.md
2. [If from /gs-brainstorm] Run /gs-map-systems to decompose your concept into individual systems
3. [If from /gs-brainstorm] Run /gs-design-system to author per-system GDDs (guided, section-by-section)
4. [If from /gs-brainstorm] Run /gs-prototype [core-mechanic] to validate the core idea before writing GDDs
5. [If fresh start] Run /gs-brainstorm to discover your game concept
6. Create your first milestone: /gs-sprint-plan new
```

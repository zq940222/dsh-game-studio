...
**Distinct from `/gs-project-stage-detect`**: That skill is diagnostic ("where are we?").
...
- **With argument**: `/gs-gate-check production` — validate readiness for that specific phase
- **No argument**: Auto-detect current stage using the same heuristics as
  `/gs-project-stage-detect`, then **confirm with the user before running**:
...
- [ ] Concept prototype exists in `prototypes/` with a REPORT.md showing PROCEED verdict
      (`/gs-prototype [core-mechanic]`) — skipping this means GDDs may be written for an
      idea that hasn't been played. Acceptable if the concept is proven by other means.

**Quality Checks:**
- [ ] Game concept has been reviewed (`/gs-design-review` verdict not MAJOR REVISION NEEDED)
...
- [ ] All MVP-tier GDDs exist in `design/gdd/` and individually pass `/gs-design-review`
- [ ] A cross-GDD review report exists in `design/gdd/` (from `/gs-review-all-gdds`)

**Quality Checks:**
- [ ] All MVP GDDs pass individual design review (8 required sections, no MAJOR REVISION NEEDED verdict)
- [ ] `/gs-review-all-gdds` verdict is not FAIL (cross-GDD consistency and design theory checks pass)
- [ ] All cross-GDD consistency issues flagged by `/gs-review-all-gdds` are resolved or explicitly accepted
...
- [ ] Technical preferences configured (`../../handbook/technical-preferences.md` populated)
...
- [ ] Engine reference docs exist in `../../engines/[engine]/`
...
- [ ] No ADR references APIs listed in `../../engines/[engine]/deprecated-apis.md`
...
**Engine Validation** (read `../../engines/[engine]/VERSION.md` first):
...
**Required Artifacts:**
- [ ] Vertical slice exists in `prototypes/` with a REPORT.md (run `/gs-vertical-slice`) — **recommended, not blocking**; if absent, surface as CONCERNS
...
- [ ] Entity inventory exists at `design/assets/entity-inventory.md` (recommended — run `/gs-asset-spec` with no arguments to generate collaboratively from GDDs + art bible)
...
- [ ] Control manifest exists at `docs/architecture/control-manifest.md`
      (generated by `/gs-create-control-manifest` from Accepted ADRs)
- [ ] Epics defined in `production/epics/` with at least Foundation and Core
      layer epics present (use `/gs-create-epics layer: foundation` and
      `/gs-create-epics layer: core` to create them, then `/gs-create-stories [epic-slug]`
      for each epic)
...
- [ ] All key screen UX specs have passed `/gs-ux-review` (verdict APPROVED or NEEDS REVISION accepted)
...
- [ ] Manual validation confirms GDDs + architecture + epics are coherent
      (run `/gs-review-all-gdds` and `/gs-architecture-review` if not done recently)
...
- [ ] QA plan exists in `production/qa/` (generated by `/gs-qa-plan`) covering this sprint or final production sprint
- [ ] At least one QA plan exists in `production/qa/` covering this production phase — run `/gs-qa-plan` if missing (CONCERNS — advisory, not blocking)
- [ ] QA sign-off report exists in `production/qa/` (generated by `/gs-team-qa`) with verdict APPROVED or APPROVED WITH CONDITIONS
...
- [ ] QA test plan exists (`/gs-qa-plan` output in `production/qa/`)
- [ ] QA sign-off report exists (`/gs-team-qa` output — APPROVED or APPROVED WITH CONDITIONS)
...
- [ ] Balance data has been reviewed (`/gs-balance-check` run)
- [ ] Release checklist completed (`/gs-release-checklist` or `/gs-launch-checklist` run)
...
Use `glob('design/gdd/gdd-cross-review-*.md')` to find the `/gs-review-all-gdds` report.
If no file matches, mark the "cross-GDD review report exists" artifact as **FAIL** and
surface it prominently: "No `/gs-review-all-gdds` report found in `design/gdd/`. Run
`/gs-review-all-gdds` before advancing to Technical Setup."
...
- "Performance profiling data isn't available. Would you like to run `/gs-perf-profile`?"
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via Task using the parallel gate protocol from `../../handbook/director-gates.md`. Issue all four Task calls simultaneously — do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (`../../handbook/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`../../handbook/director-gates.md`)
3. **`producer`** — gate **PR-PHASE-GATE** (`../../handbook/director-gates.md`)
4. **`art-director`** — gate **AD-PHASE-GATE** (`../../handbook/director-gates.md`)
...
### Blockers
1. **No Architecture Decision Records** — Run `/gs-architecture-decision` to create one
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `ask_user_question`.
...
[A] Run /gs-create-architecture — produce your master architecture blueprint and ADR work plan (recommended next step)
[B] Design more GDDs first — return here when all MVP systems are complete
[C] Stop here for this session
```

> **Note for systems-design PASS**: `/gs-create-architecture` is the required next step before writing any ADRs. It produces the master architecture document and a prioritized list of ADRs to write. Running `/gs-architecture-decision` without this step means writing ADRs without a blueprint — skip it at your own risk.

For **technical-setup PASS**:
```
Gate passed. What would you like to do next?
[A] Run /gs-create-control-manifest — generate the layer rules manifest from your Accepted ADRs (do this first)
[B] Run /gs-vertical-slice — build the Vertical Slice (do this before writing epics — validate fun first)
[C] Write more ADRs first — run /gs-architecture-decision [next-system]
...
>
> 1. `/gs-create-control-manifest` — extract technical rules from Accepted ADRs (required before epics)
> 2. `/gs-vertical-slice` — build the Vertical Slice **FIRST**, before writing epics or stories
> 3. Playtest → `/gs-playtest-report` — at least 1 session required to pass the Pre-Production gate; 3+ recommended before committing the full team
> 4. `/gs-ux-design [screen]` — UX specs for main menu, core HUD, pause menu (if not done)
> 5. `/gs-create-epics layer:foundation` then `/gs-create-epics layer:core` — plan after fun is validated
> 6. `/gs-create-stories [epic-slug]` for each epic
> 7. `/gs-sprint-plan new`
>
...
- **No art bible?** → `/gs-art-bible` to create the visual identity specification
- **Art bible exists but no asset specs?** → `/gs-asset-spec system:[name]` to generate per-asset visual specs and generation prompts from approved GDDs
- **No game concept?** → `/gs-brainstorm` to create one
- **No systems index?** → `/gs-map-systems` to decompose the concept into systems
- **Missing design docs?** → `/gs-reverse-document` or delegate to `game-designer`
- **Small design change needed?** → `/gs-quick-design` for changes under ~4 hours (bypasses full GDD pipeline)
- **No UX specs?** → `/gs-ux-design [screen name]` to author specs, or `/gs-team-ui [feature]` for full pipeline
- **UX specs not reviewed?** → `/gs-ux-review [file]` or `/gs-ux-review all` to validate
- **No accessibility requirements doc?** → run `/gs-ux-design` which creates both `design/accessibility-requirements.md` and `design/ux/interaction-patterns.md` in one step
- **No interaction pattern library?** → `/gs-ux-design patterns` to initialize it
- **GDDs not cross-reviewed?** → `/gs-review-all-gdds` (run after all MVP GDDs are individually approved)
- **Cross-GDD consistency issues?** → fix flagged GDDs, then re-run `/gs-review-all-gdds`
- **No test framework?** → `/gs-test-setup` to scaffold the framework for your engine
- **No QA plan for current sprint?** → `/gs-qa-plan sprint` to generate one before implementation begins
- **Missing ADRs?** → `/gs-architecture-decision` for individual decisions
- **No master architecture doc?** → `/gs-create-architecture` for the full blueprint
- **ADRs missing engine compatibility sections?** → Re-run `/gs-architecture-decision`
  or manually add Engine Compatibility sections to existing ADRs
- **Missing control manifest?** → `/gs-create-control-manifest` (requires Accepted ADRs)
- **Missing epics?** → `/gs-create-epics layer: foundation` then `/gs-create-epics layer: core` (requires control manifest)
- **Missing stories for an epic?** → `/gs-create-stories [epic-slug]` (run after each epic is created)
- **Stories not implementation-ready?** → `/gs-story-readiness` to validate stories before developers pick them up
- **Tests failing?** → delegate to `lead-programmer` or `qa-tester`
- **No playtest data?** → `/gs-playtest-report`
- **No playtest sessions beyond the minimum?** → Additional sessions give more reliable signal. 3+ total is recommended before committing the full team. Use `/gs-playtest-report` to structure findings.
- **No Difficulty Curve doc?** → Create `design/difficulty-curve.md` from the template at `../../templates/difficulty-curve.md` — or use `/gs-quick-design "difficulty curve"` for a guided session.
- **No player journey map?** → Create `design/player-journey.md` from the template at `../../templates/player-journey.md` — or author it collaboratively using `/gs-ux-design` Phase 2b.
- **Need a quick sprint check?** → `/gs-sprint-status` for current sprint progress snapshot
- **Performance unknown?** → `/gs-perf-profile`
- **Not localized?** → `/gs-localize`
- **Ready for release?** → `/gs-launch-checklist`
...
6. **Never auto-fix**: If required artifacts are missing, report the FAIL verdict and
   name the skill to run (e.g. "run `/gs-test-setup`"). Do NOT create missing files or

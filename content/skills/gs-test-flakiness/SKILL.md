...
- After `/gs-regression-suite` identifies quarantined tests that need diagnosis
...
**Modes:**
- `/gs-test-flakiness [ci-log-path]` — analyse a specific CI run log file
- `/gs-test-flakiness scan` — scan all available CI logs in `.github/` or
  standard log output directories
- `/gs-test-flakiness registry` — read existing regression-suite.md quarantine
...
> 3. Run `/gs-test-flakiness registry` to review tests already flagged as flaky

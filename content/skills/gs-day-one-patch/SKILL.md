...
If [C]: output "No day-one patch required. Proceed to `/gs-launch-checklist`." Stop.
...
Use `ask_user_question` to confirm proceeding or reduce scope.
...
Spawn `release-manager` via a subagent. Ask them to produce a rollback plan covering:
...
1. Spawn `lead-programmer` via a subagent with:
...
3. Spawn `qa-tester` via a subagent to verify: does the bug reproduce after the fix?
...
This is a lightweight QA pass — not a full `/gs-team-qa`. The patch is already QA-approved from the release gate; we are only re-verifying the changed areas.

Spawn `qa-lead` via a subagent with:
...
- **Targeted smoke check** — run `/gs-smoke-check [affected-systems]`
...
1. Run `/gs-patch-notes` to generate the player-facing version of the patch notes
2. Run `/gs-bug-report verify [BUG-ID]` for each fixed bug after the patch is live
3. Run `/gs-bug-report close [BUG-ID]` for each verified fix
4. Schedule a post-launch review 48–72 hours after launch using `/gs-retrospective launch`
...
- Options:
  - `[A] Run /gs-patch-notes — generate player-facing patch notes`
  - `[B] Run /gs-bug-report to log any issues found post-deploy`
  - `[C] Stop here`
...
- **Player communication is part of the patch** — `/gs-patch-notes` is a required output, not optional

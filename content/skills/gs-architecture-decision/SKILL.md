...
(e.g., `/gs-architecture-decision retrofit docs/architecture/adr-0001-event-system.md`):

Enter **retrofit mode**:

1. Read the existing ADR file completely.
2. Identify which template sections are present by scanning headings:
   - `## Status` — **BLOCKING if missing**: `/gs-story-readiness` cannot check ADR acceptance
...
7. Suggest: "Run `/gs-architecture-review` to re-validate coverage now that this ADR
...
1. Read `../../engines/[engine]/VERSION.md` to get:
...
3. Read the corresponding module reference if it exists:
   `../../engines/[engine]/modules/[domain].md`

4. Read `../../engines/[engine]/breaking-changes.md` — flag any
   changes in the relevant domain that post-date the LLM's training cutoff.

5. Read `../../engines/[engine]/deprecated-apis.md` — flag any APIs
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
   ...
   Run `/gs-setup-engine` first, or tell me which engine you are using."
...
a **confirm/adjust** prompt using `ask_user_question` — not open-ended questions.
...
decisions remain, present each one as a separate `ask_user_question` with the proposed
...
| **References Consulted** | [List engine-reference docs read, e.g. `../../engines/godot/modules/physics.md`] |
...
5. **Write approval** — Use `ask_user_question`:
...
After the ADR is written (and registry optionally updated), close with `ask_user_question`.
...
[N] Start writing GDDs — run `/gs-design-system [first-undesigned-system]` (only show if all prerequisite ADRs are written)
[N+1] Stop here for this session
```

If there are no remaining priority ADRs and no undesigned GDD systems, offer only "Stop here" and suggest running `/gs-architecture-review` in a fresh session.

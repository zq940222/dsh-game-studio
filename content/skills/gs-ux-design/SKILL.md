...
**If no argument is provided**, do not fail — ask instead. Use `ask_user_question`:
...
- **Game concept**: Read `design/gdd/game-concept.md` — if missing, warn:
  > "No game concept found. Run `/gs-brainstorm` first to establish the game's
...
> "Player journey map not yet created. Template available at `../../templates/player-journey.md`. Run `/gs-ux-design` Phase 2b or create it manually to establish player context for this screen."
...
Read `../../handbook/technical-preferences.md` and extract the `## Input & Platform`
...
>
> (Run `/gs-setup-engine` to save this permanently so you won't be asked again.)
...
2. **Questions**: Ask what is needed to draft this section. Use `ask_user_question`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `ask_user_question` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `ask_user_question`:
...
7. **Write**: Use `ask_user_question`: "May I write the [section name] section to `[filepath]`?"
...
- Use `ask_user_question` to capture the choice:
...
- Use `ask_user_question`: "Want an ASCII wireframe as part of this spec?"
...
> "Accessibility tier not yet defined — consider WCAG-AA as a baseline. Run `/gs-gate-check` to see whether this blocks any phase gates."
...
Write at least 5 specific, testable criteria that a QA tester can verify without reading any other design document. These become the pass/fail conditions for `/gs-story-done`.
...
Use `ask_user_question` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `ask_user_question`:
...
> "This spec should be validated with `/gs-ux-review` before it enters the
...
- "Run `/gs-ux-review [filename]` now, or do something else first?"
  - Options:
    - "Run `/gs-ux-review` now — validate this spec"
...
`/gs-ux-review` on all completed specs before running `/gs-gate-check pre-production`."
...
- Run `/gs-ux-review [filename]` to validate this spec before it enters the implementation pipeline
- Run `/gs-ux-design [next-screen]` to continue designing remaining screens or flows
- Run `/gs-gate-check pre-production` once all key screens have approved UX specs

---

Usually executed by the `ux-designer` role. Load `gs-roster` for the delegation protocol.

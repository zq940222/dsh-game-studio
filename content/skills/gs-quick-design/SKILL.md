...
Full GDD authoring via `/gs-design-system` is the heavyweight path. Use this skill
...
**When to run:** Anytime a change is too small for `/gs-design-system` but too
...
- Options:
  ...
  - `[F] This is too large — redirect me to /gs-design-system`

If [F]: stop. Verdict: **REDIRECTED** — use `/gs-design-system` for this change.
...
Present the draft to the user in full. Then use `ask_user_question`:
...
- Options:
  ...
  - `[C] This grew too large — redirect to /gs-design-system instead`

If [B]: collect the requested changes, revise the draft, and re-present this widget.
If [C]: stop. Verdict: **REDIRECTED** — use `/gs-design-system` for this change.
...
Next step: This spec is ready for `/gs-story-readiness` validation before
...
Quick Design Specs **bypass** `/gs-design-review` and `/gs-review-all-gdds` by
...
using `/gs-design-system` to author a full GDD for this."

---

## Recommended Next Steps

- Run `/gs-story-readiness [story-path]` to validate the story before implementation begins — reference this spec in the story's GDD Reference field
- Run `/gs-dev-story [story-path]` to implement once the story passes readiness checks
- If the change is larger than expected, run `/gs-design-system [system-name]` to author a full GDD instead

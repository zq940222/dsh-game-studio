...
### Issues
- [N] hardcoded strings found in source code (run /gs-localize scan)
...
Spawn `localization-lead` via a subagent. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
Read `../../handbook/technical-preferences.md` to determine the engine. Then check:
...
[ ] /gs-localize scan shows zero hardcoded strings
[ ] /gs-localize validate shows no placeholder mismatches in source (en)
...
## Post-Freeze Changes
[Any strings added or modified after freeze are listed here automatically by /gs-localize extract]
```
...
Spawn `localization-lead` via a subagent with:
- The target locale(s) to QA
- The list of all screens/flows in the game (from `design/gdd/` or `/gs-content-audit` output)
- The current `/gs-localize validate` report
...
```
/gs-localize scan            → find hardcoded strings
/gs-localize extract         → build string table
/gs-localize freeze          → lock source before sending to translators
/gs-localize brief           → generate translator briefing document
[Send to translators]
/gs-localize validate        → check returned translations
/gs-localize cultural-review → flag culturally sensitive content
/gs-localize rtl-check       → if shipping Arabic / Hebrew / Persian
/gs-localize vo-pipeline     → if shipping dubbed VO
/gs-localize qa              → full localization QA pass
```

After `qa` returns PASS for all shipping locales, include the QA report path when running `/gs-gate-check release`.

---

Usually executed by the `localization-lead` role. Load `gs-roster` for the delegation protocol.

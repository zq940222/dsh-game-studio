---
name: gs-dev-story
description: Read a story file and implement it. Loads the full context (story, GDD requirement, ADR guidelines, control manifest), routes to the right programmer agent for the system and engine, implements the code and test, and confirms each acceptance criterion. The core implementation skill — run after /gs-story-readiness, before /gs-code-review and /gs-story-done.
...
```
/gs-qa-plan sprint           ← define test requirements before sprint begins
/gs-story-readiness [path]   ← validate before starting
/gs-dev-story [path]         ← implement it  (this skill)
/gs-code-review [files]      ← review it
/gs-story-done [path]        ← verify and close it
```

**After all sprint stories are done:** run `/gs-team-qa sprint` to execute the full QA cycle and get a sign-off verdict before advancing the project stage.
...
|------|------|------------|
| TR registry | `docs/architecture/tr-registry.yaml` | **STOP** — "TR registry not found at `docs/architecture/tr-registry.yaml`. Run `/gs-architecture-review` to bootstrap the registry from your GDDs and ADRs." |
| Governing ADR | path from story's ADR field | **STOP** — "ADR file [path] not found. Run `/gs-architecture-decision` to create it, or correct the filename in the story's ADR field." |
| Control manifest | `docs/architecture/control-manifest.md` | **WARN and continue** — "Control manifest not found — layer rules cannot be checked. Run `/gs-create-control-manifest`." |
...
If they differ, use `ask_user_question` before proceeding:
...
If [B]: edit the story file's `Manifest Version:` field to the current manifest date AND add a `Manifest-Note: Proceeded with old manifest rules on [date] — non-compliance risk accepted.` line to the story header. Read the manifest for new rules anyway. Note the decision in the Phase 6 summary under "Deviations". `/gs-story-done` will include the Manifest-Note in its deviations section without re-checking staleness.
If [C]: stop. Do not spawn any agent. Let the user review and re-run `/gs-dev-story`.
...
### Engine reference
Read `../../handbook/technical-preferences.md`:
...
specialist to spawn via a subagent.
...
Read the `Engine Specialists` section of `../../handbook/technical-preferences.md`
...
Spawn the chosen programmer agent(s) via a subagent with the full context package:
...
5. **Engine preferences**: `../../handbook/technical-preferences.md` — read naming conventions and performance budgets
6. **Test file path**: `[path from story's Test Evidence section]` — this file must be created as part of implementation
7. **Test requirement** (Logic and Integration stories only): The test file MUST be created at `[path from the story's Test Evidence section]`. Write the test alongside the implementation — do not defer it. The story cannot be closed via `/gs-story-done` without this file present. Each acceptance criterion must have at least one test function covering it. Test file naming: `[system]_[feature]_test.[ext]`. Function naming: `test_[scenario]_[expected_outcome]`. No random seeds, no time-dependent assertions, no external I/O.
...
check happens in `/gs-story-done` via manual confirmation.
...
**Before running `/gs-story-done`:** run your test suite locally and confirm the tests you wrote pass. `/gs-story-done` will re-run them automatically, but a failing test discovered there means returning to implementation context.

Ready for: `/gs-code-review [file1] [file2]` then `/gs-story-done [story-path]`
```
...
- **File writes are delegated** — all source code, test files, and evidence docs are written by sub-agents spawned via a subagent. Each sub-agent enforces the "May I write to [path]?" protocol individually. This orchestrator does not write files directly.
...
- **Visual/Feel criteria are deferred, not skipped** — mark them as DEFERRED
  in the summary; they will be manually verified in `/gs-story-done`
...
- Run `/gs-code-review [file1] [file2]` to review the implementation before closing the story
- Run `/gs-story-done [story-path]` to verify acceptance criteria and mark the story complete
- After all sprint stories are done: run `/gs-team-qa sprint` for the full QA cycle before advancing the project stage

---
name: gs-create-epics
description: Translate approved GDDs + architecture into epics — one epic per architectural module. Defines scope, governing ADRs, engine risk, and untraced requirements. Does NOT break into stories — run /gs-create-stories [epic-slug] after each epic is created.
...
**Next step after each epic:** `/gs-create-stories [epic-slug]`

**When to run:** After `/gs-create-control-manifest` and `/gs-architecture-review` pass.
...
**Modes:**
- `/gs-create-epics all` — process all systems in layer order
- `/gs-create-epics layer: foundation` — Foundation layer only
- `/gs-create-epics layer: core` — Core layer only
- `/gs-create-epics layer: feature` — Feature layer only
- `/gs-create-epics layer: presentation` — Presentation layer only
- `/gs-create-epics [system-name]` — one specific system
...
- `../../engines/[engine]/VERSION.md` — engine name, version, risk levels
...
> Run `/gs-architecture-decision` first, or proceed with placeholders."
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` via a subagent using gate **PR-EPIC** (`../../handbook/director-gates.md`).
...
If CONCERNS, use `ask_user_question`:
...
> **Status**: Ready
> **Stories**: Not yet created — run `/gs-create-stories [epic-slug]`
...
- All stories are implemented, reviewed, and closed via `/gs-story-done`
...
Run `/gs-create-stories [epic-slug]` to break this epic into implementable stories.
```
...
- **Foundation + Core complete**: These are required for the Pre-Production →
  Production gate. Run `/gs-gate-check production` to check readiness.
- **Reminder**: Epics define scope. Stories define implementation steps. Run
  `/gs-create-stories [epic-slug]` for each epic before developers can pick up work.
...
- **Verdict: COMPLETE** — [N] epic(s) written. Run `/gs-create-stories [epic-slug]` per epic.

...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `/gs-release-checklist`. Do not auto-invoke based on context matching.
...
- Run `/gs-gate-check` for a formal phase gate verdict before proceeding to release.
- Coordinate final sign-offs via `/gs-team-release`.

...
> Usage: `/gs-team-narrative [narrative content description]` — describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`). Do not use `ask_user_question` here; output the guidance directly.
...
- delegate to `narrative-director` (the child reads `roles/narrative-director.md` itself) — Story arcs, character design, narrative vision
- delegate to `writer` (the child reads `roles/writer.md` itself) — Dialogue writing, lore entries, in-game text
- delegate to `world-builder` (the child reads `roles/world-builder.md` itself) — World rules, faction design, history, geography
- delegate to `art-director` (the child reads `roles/art-director.md` itself) — Character visual profiles, environmental visual storytelling, cinematic tone
- delegate to `level-designer` (the child reads `roles/level-designer.md` itself) — Level layouts that serve the narrative, pacing
- delegate to `localization-lead` (the child reads `roles/localization-lead.md` itself) — Localization readiness — flags non-localizable strings, cultural assumptions, and i18n gaps
...
sub-agents. Each sub-agent enforces the "May I write to [path]?"

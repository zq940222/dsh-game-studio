...
`/gs-sprint-plan update` or `/gs-milestone-review`.
...
- If an argument is given (e.g., `/gs-sprint-status 3`), search
...
- If `production/sprints/` does not exist or is empty, report: "No sprint
  files found. Start a sprint with `/gs-sprint-plan new`." Then stop.
...
"⚠ No `sprint-status.yaml` found — status inferred from markdown. Run `/gs-sprint-plan update` to generate one."
...
time remaining. Recommend replanning with `/gs-sprint-plan update`.
```
...
Run `/gs-story-readiness sprint` to validate story file coverage.
```
...
- It does not propose scope cuts (that is `/gs-sprint-plan update`)
- It makes at most one recommendation per run

For more detail on a specific story, the user can read the story file directly
or run `/gs-story-readiness [path]`.

For sprint replanning, use `/gs-sprint-plan update`.
For end-of-sprint retrospective, use `/gs-retrospective`.

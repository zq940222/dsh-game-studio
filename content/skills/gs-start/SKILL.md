...
Check:
- **Engine configured?** Read `../../handbook/technical-preferences.md`. If the Engine field contains `[TO BE CONFIGURED]`, the engine is not set.
...
- **Prompt**: "Welcome to the Game Studio! Before I suggest anything, I'd like to understand where you're starting from. Where are you at with your game idea right now?"
- **Options**:
...
2. Briefly explain what `/gs-brainstorm` does (guided ideation using professional frameworks — MDA, player psychology, verb-first design). Mention that it has two modes: `/gs-brainstorm open` for fully open exploration, or `/gs-brainstorm [hint]` if they have even a vague theme (e.g., "space", "cozy", "horror").
3. Recommend running `/gs-brainstorm open` as the next step, but invite them to use a hint if something comes to mind
4. Show the recommended path:
   **Concept phase:**
   - `/gs-brainstorm open` — discover your game concept
   - `/gs-setup-engine` — configure the engine (brainstorm will recommend one)
   - `/gs-prototype` — throwaway concept build: validate the core idea is fun before designing (1–3 days)
   - `/gs-art-bible` — define visual identity (uses the Visual Identity Anchor brainstorm produces)
   - `/gs-map-systems` — decompose the concept into systems
   - `/gs-design-system` — author a GDD for each MVP system
   - `/gs-review-all-gdds` — cross-system consistency check
   - `/gs-gate-check` — validate readiness before architecture work
   **Architecture phase:**
   - `/gs-create-architecture` — produce the master architecture blueprint and Required ADR list
   - `/gs-architecture-decision (×N)` — record key technical decisions, following the Required ADR list
   - `/gs-create-control-manifest` — compile decisions into an actionable rules sheet
   - `/gs-architecture-review` — validate architecture coverage
   **Pre-Production phase:**
   - `/gs-ux-design` — author UX specs for key screens (main menu, HUD, core interactions)
   - `/gs-vertical-slice` — production-quality end-to-end build to validate the full game loop
   - `/gs-playtest-report (×1+)` — document each vertical slice playtest session
   - `/gs-create-epics` — map systems to epics
   - `/gs-create-stories` — break epics into implementable stories
   - `/gs-sprint-plan` — plan the first sprint
   **Production phase:** → pick up stories with `/gs-dev-story`
...
3. Recommend running `/gs-brainstorm [their hint]` to develop it
4. Show the recommended path:
   **Concept phase:**
   - `/gs-brainstorm [hint]` — develop the idea into a full concept
   - `/gs-setup-engine` — configure the engine
   - `/gs-prototype` — throwaway concept build: validate the core idea is fun before designing (1–3 days)
   - `/gs-art-bible` — define visual identity (uses the Visual Identity Anchor brainstorm produces)
   - `/gs-map-systems` — decompose the concept into systems
   - `/gs-design-system` — author a GDD for each MVP system
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not ask_user_question (it's an open response).
2. Acknowledge the concept, then use `ask_user_question` to offer two paths:
   - **Prompt**: "How would you like to proceed?"
   - **Options**:
     - `Formalize it first` — Run `/gs-brainstorm [concept]` to structure it into a proper game concept document
     - `Jump straight in` — Go to `/gs-setup-engine` now and write the GDD manually afterward
3. Show the recommended path:
   **Concept phase:**
   - `/gs-brainstorm` or `/gs-setup-engine` — (their pick from step 2)
   - `/gs-prototype` — throwaway concept build: validate the core idea is fun before designing (1–3 days)
   - `/gs-art-bible` — define visual identity (after brainstorm if run, or after concept doc exists)
   - `/gs-design-review` — validate the concept doc
   - `/gs-map-systems` — decompose the concept into individual systems
   - `/gs-design-system` — author a GDD for each MVP system
   - `/gs-review-all-gdds` — cross-system consistency check
   - `/gs-gate-check` — validate readiness before architecture work
   **Architecture phase:**
...
2. **Sub-case D1 — Early stage** (engine not configured or only a game concept exists):
   - Recommend `/gs-setup-engine` first if engine not configured
   - Then `/gs-project-stage-detect` for a gap inventory

   **Sub-case D2 — GDDs, ADRs, or stories already exist:**
   - Explain: "Having files isn't the same as the template's skills being able to use them. GDDs might be missing required sections. `/gs-adopt` checks this specifically."
   - Recommend:
     1. `/gs-project-stage-detect` — understand what phase and what's missing entirely
     2. `/gs-adopt` — audit whether existing artifacts are in the right internal format

3. Show the recommended path for D2:
   - `/gs-project-stage-detect` — phase detection + existence gaps
   - `/gs-adopt` — format compliance audit + migration plan
   - `/gs-setup-engine` — if engine not configured
   - `/gs-design-system retrofit [path]` — fill missing GDD sections
   - `/gs-architecture-decision retrofit [path]` — add missing ADR sections
   - `/gs-architecture-review` — bootstrap the TR requirement registry
   - `/gs-gate-check` — validate readiness for next phase
...
- **Options**:
  ...
  - `Lean (recommended)` — Directors only at phase gate transitions (/gs-gate-check). Skips per-skill reviews. Balanced approach for solo devs and small teams.
...
When the user confirms their next step, respond with a single short line: "Type `[skill command]` to begin." Nothing else. Do not re-explain the skill or add encouragement. The `/gs-start` skill's job is done.

...
If a matching file is found, use `ask_user_question`:
...
> "No sprint data found for [sprint/milestone]. Run `/gs-sprint-status` to generate
> sprint data first, or provide the sprint details manually."

Then use `ask_user_question` to present two options:
...
If the user selects [A]: Proceed to invoke `/gs-sprint-plan new`, passing the retrospective file path and a summary of the action items and velocity change so the sprint planner can reference them.

- If this was a milestone retrospective, run `/gs-gate-check` to formally assess readiness for the next phase.

...
> internal changelog, then re-run `/gs-patch-notes [version]`."
...
1. Check `../../handbook/technical-preferences.md` for any "tone", "voice", or "style"
   fields or sections.
...
- Run `/gs-release-checklist` to verify all other release gates are met before publishing.
- Share the patch notes draft with the community-manager for tone review before posting publicly.

---

Usually executed by the `community-manager` role. Load `gs-roster` for the delegation protocol.

---
name: gs-create-control-manifest
...
**When to run:** After `/gs-architecture-review` passes and ADRs are in Accepted
...
### Engine Reference
- Read `../../engines/[engine]/VERSION.md` for engine + version
- Read `../../engines/[engine]/deprecated-apis.md` — these become
  forbidden API entries
- Read `../../engines/[engine]/current-best-practices.md` if it exists
...
Spawn `technical-director` via a subagent using gate **TD-MANIFEST** (`../../handbook/director-gates.md`).
...
- **CONCERNS** → surface via `ask_user_question` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
...
> **Status**: [Active — regenerate with `/gs-create-control-manifest update` when ADRs change]

`Manifest Version` is the date this manifest was generated. Story files embed
this date when created. `/gs-story-readiness` compares a story's embedded version
...
- Source: `../../engines/[engine]/deprecated-apis.md`
...
- If epics/stories don't exist yet: "Run `/gs-create-epics layer: foundation` then `/gs-create-stories [epic-slug]` — programmers

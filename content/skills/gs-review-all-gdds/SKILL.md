...
**This is distinct from `/gs-design-review`**, which reviews one GDD for internal
...
- Before `/gs-create-architecture` begins (architecture built on inconsistent GDDs
...
Run `/gs-consistency-check` after this review to populate the registry."
...
> then re-run `/gs-review-all-gdds`."
...
Use `ask_user_question` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `ask_user_question`:
...
After all file writes are complete, use `ask_user_question` for a closing widget.
...
- Are any GDDs in the "Flagged for Revision" table? → offer /gs-design-review option for each
- Read systems-index.md for the next system with Status: Not Started → offer /gs-design-system option
- Is the verdict PASS or CONCERNS? → offer /gs-gate-check or /gs-create-architecture
...
- `[_] Run /gs-design-review [flagged-gdd-path] — address flagged warnings` (one per flagged GDD, if any)
- `[_] Run /gs-design-system [next-system] — next in design order` (always include, name the actual system)
- `[_] Run /gs-create-architecture — begin architecture (verdict is PASS/CONCERNS)` (include if verdict is not FAIL)
- `[_] Run /gs-gate-check — validate Systems Design phase gate` (include if verdict is PASS)
- `[_] Stop here`

...
> "Usage: `/gs-team-live-ops [season name or event description]` — Provide the name or description of the season or live event to plan."
...
- delegate to `live-ops-designer` (the child reads `roles/live-ops-designer.md` itself) — Season/event structure and retention mechanics
- delegate to `economy-designer` (the child reads `roles/economy-designer.md` itself) — Live economy balance and reward pricing
- delegate to `analytics-engineer` (the child reads `roles/analytics-engineer.md` itself) — Success metrics, A/B tests, event instrumentation
- delegate to `community-manager` (the child reads `roles/community-manager.md` itself) — Player-facing communication and messaging
- delegate to `narrative-director` (the child reads `roles/narrative-director.md` itself) — Seasonal theme and narrative framing
- delegate to `writer` (the child reads `roles/writer.md` itself) — All player-facing text: event descriptions, item names, copy

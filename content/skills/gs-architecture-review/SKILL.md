---
name: gs-architecture-review
description: Validates completeness and consistency of the project architecture against all GDDs. Builds a traceability matrix mapping every GDD technical requirement to ADRs, identifies coverage gaps, detects cross-ADR conflicts, verifies engine compatibility consistency across all decisions, and produces a PASS/CONCERNS/FAIL verdict. The architecture equivalent of /gs-design-review.
...
- `../../engines/[engine]/breaking-changes.md`
- `../../engines/[engine]/deprecated-apis.md`
- All files in `../../engines/[engine]/modules/`

### Project Standards
- `../../handbook/technical-preferences.md`
...
After completing the engine audit above, spawn the **primary engine specialist** via a subagent for a domain-expert second opinion:
...
- Spawn a subagent for `[primary specialist]` (the child reads its own brief under `roles/`) with: all ADRs that contain engine-specific decisions or `Post-Cutoff APIs Used` fields, the engine reference docs, and the Phase 5 audit findings. Ask them to:
...
If [B]: display the complete proposed systems-index section, then re-ask with `ask_user_question`.
...
For each gap:
  ❌ TR-[id]: [GDD] → [system] → [requirement]
     Suggested ADR: "/gs-architecture-decision [suggested title]"
...
Use `ask_user_question` for the write approval:
...
For `rtm` mode, use `ask_user_question`:
...
> Last Updated: [date]
> Mode: /gs-architecture-review rtm
...
```markdown
### [YYYY-MM-DD] — /gs-architecture-review — 🔴 CONFLICT
...
2. **Pre-gate checklist**: Check whether these exist via glob and mark each ✅ or ❌:
   - `tests/unit/` and `tests/integration/` directories — if ❌: run `/gs-test-setup`
   - `.github/workflows/tests.yml` — if ❌: run `/gs-test-setup`
   - `design/accessibility-requirements.md` — if ❌: run `/gs-ux-design`
   - `design/ux/interaction-patterns.md` — if ❌: run `/gs-ux-design`
   Present ❌ items as required steps before gate-check. Do not offer `/gs-gate-check`
   as an option if any item is ❌ — offer the missing skill to run instead.
3. **Rerun trigger**: "Re-run `/gs-architecture-review` after each new ADR is written
   to verify coverage improves"

Then close with `ask_user_question` tailored to the pre-gate checklist state:
- If ADR gaps remain or any pre-gate item is ❌:
  - "Architecture review complete. What would you like to do next?"
    - [A] Write a missing ADR — open a fresh session and run `/gs-architecture-decision [system]`
    - [B] Run `/gs-test-setup` — required before gate-check (only show if test infrastructure is ❌)
    - [C] Run `/gs-ux-design` — required before gate-check (only show if UX/accessibility files are ❌)
...
- If all pre-gate checklist items are ✅ and no blocking ADR gaps remain:
  - "Architecture review complete. All pre-gate items confirmed. What would you like to do next?"
    - [A] Run `/gs-gate-check pre-production`
    - [B] Write a missing ADR — open a fresh session and run `/gs-architecture-decision [system]`
...
3. **Offer options** via ask_user_question with three choices:

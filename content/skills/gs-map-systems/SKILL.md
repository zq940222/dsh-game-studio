...
- **No argument**: `/gs-map-systems` — Run the full decomposition workflow (Phases 1-5)
  to create or update the systems index.
- **`next`**: `/gs-map-systems next` — Pick the highest-priority undesigned system
...
- Read `design/gdd/game-concept.md` — **fail with a clear message if missing**:
  > "No game concept found at `design/gdd/game-concept.md`. Run `/gs-brainstorm` first
...
Then use `ask_user_question` to capture feedback:
...
Use `ask_user_question` to ask: "Does this dependency ordering look right? Any
...
Use `ask_user_question` to ask: "Do these priority assignments match your vision?
...
Using the template at `../../templates/systems-index.md`, populate the
...
## Phase 6: Design Individual Systems (Handoff to /gs-design-system)

This phase is entered when:
- The user says "yes" to designing systems after creating the index
- The user invokes `/gs-map-systems [system-name]`
- The user invokes `/gs-map-systems next`
...
Use `ask_user_question` for: "Start designing [system-name] now, pick a different
system, or stop here?"

### Step 6b: Hand Off to /gs-design-system

Once a system is selected, invoke the `/gs-design-system [system-name]` skill.

The `/gs-design-system` skill handles the full GDD authoring process:
...
- Runs `/gs-design-review` when complete
- Updates the systems index

**Do not duplicate the /gs-design-system workflow here.** This skill owns the systems
*index*; `/gs-design-system` owns individual system *GDDs*.

### Step 6c: Loop or Stop

After `/gs-design-system` completes, use `ask_user_question`:
...
After the systems index is created (or after designing some systems), present next actions using `ask_user_question`:

- "Systems index is written. What would you like to do next?"
  - [A] Start designing GDDs — run `/gs-design-system [first-system-in-order]`
  - [B] Run `/gs-gate-check systems-design` — triggers the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates automatically for a formal director sign-off on the system set
  - [C] Stop here for this session

**The gate-check option ([B]) is worth highlighting**: running `/gs-gate-check systems-design` triggers both the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates, catching scope issues, missing systems, and boundary problems before they're locked in across many documents. It is optional but recommended for new projects.

After any individual GDD is completed:
- "Run `/gs-design-review design/gdd/[system].md` in a fresh session to validate quality"
- "Run `/gs-gate-check systems-design` when all MVP GDDs are complete"
...
2. **ask_user_question** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 6: "Start designing, pick different, or stop?" then hand off to `/gs-design-system`
...
- Run `/gs-design-system [first-system-in-order]` to author the first GDD (use design order from the index)
- Run `/gs-map-systems next` to always pick the highest-priority undesigned system automatically
- Run `/gs-design-review design/gdd/[system].md` in a fresh session after each GDD is authored

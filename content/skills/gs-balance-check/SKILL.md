...
After presenting the report, use `ask_user_question`:
...
- If the fix changes a tuning knob defined in a GDD or referenced by an ADR, remind the user:
  > "This value is defined in a design document. Run `/gs-propagate-design-change [path]` on the affected GDD to find downstream impacts before committing."
...
- Confirm the file was written, then end with: "Re-run `/gs-balance-check` after fixes to verify."

If [C]:
- Summarize open issues and end with: "Re-run `/gs-balance-check` after fixes to verify."

---

Usually executed by the `economy-designer` role. Load `gs-roster` for the delegation protocol.

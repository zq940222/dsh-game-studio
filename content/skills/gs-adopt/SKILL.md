---
name: gs-adopt
description: Brownfield onboarding — audits existing project artifacts for template format compliance (not just existence), classifies gaps by impact, and produces a numbered migration plan. Run this when joining an in-progress project or upgrading from an older template version. Distinct from /gs-project-stage-detect (which checks what exists) — this checks whether what exists will actually work with the template's skills.
...
**This is not `/gs-project-stage-detect`.**
`/gs-project-stage-detect` answers: *what exists?*
`/gs-adopt` answers: *will what exists actually work with the template's skills?*
...
- `../../handbook/technical-preferences.md` — engine configured?
- `../../engines/` — engine reference docs present?
- glob `docs/adoption-plan-*.md` — note the filename of the most recent prior plan if any exist

### Infer phase (if no stage.txt)
Use the same heuristic as `/gs-project-stage-detect`:
...
- Nothing → Fresh (not a brownfield project — suggest `/gs-start`)

If the project appears fresh (no artifacts at all), use `ask_user_question`:
- "This looks like a fresh project — no existing artifacts found. `/gs-adopt` is for
  projects with work to migrate. What would you like to do?"
  - "Run `/gs-start` — begin guided first-time onboarding"
...
|---|---|
| `## Status` | **BLOCKING** — `/gs-story-readiness` ADR status check silently passes everything |
| `## ADR Dependencies` | HIGH — dependency ordering in `/gs-architecture-review` breaks |
...
1. **Parenthetical status values** — grep for any Status cell containing
   ...
   These break exact-string matching in `/gs-gate-check`, `/gs-create-stories`,
   and `/gs-architecture-review`. **BLOCKING.**
...
| Sprint status | `production/sprint-status.yaml` | MEDIUM — `/gs-sprint-status` falls back to markdown |
| Stage file | `production/stage.txt` | MEDIUM — phase auto-detect unreliable |
| Engine reference | `../../engines/[engine]/VERSION.md` | HIGH — ADR engine checks blind |
...
Read `../../handbook/technical-preferences.md`. Check each field for `[TO BE CONFIGURED]`:
...
`/gs-architecture-decision retrofit docs/architecture/adr-[NNNN]-[slug].md`
...
`/gs-design-system retrofit design/gdd/[filename].md`
...
2. Run `/gs-architecture-review` → bootstraps `tr-registry.yaml`
3. Run `/gs-create-control-manifest` → creates manifest with version stamp
4. Run `/gs-sprint-plan update` → creates `sprint-status.yaml`
5. Run `/gs-gate-check [phase]` → writes `stage.txt` authoritatively
...
Gap counts:
  ...
  HIGH:     [N] — unsafe to run /gs-create-stories or /gs-story-readiness
...
Re-run `/gs-adopt` anytime to check remaining gaps.
...
Run `/gs-architecture-review` — even if ADRs already exist, this run bootstraps
...
Run `/gs-gate-check [current-phase]`
**Time**: 5 min
...
Run `/gs-adopt` again after completing Step 3 to verify all blocking and high gaps
...
- **Options**:
  ...
  - `Lean (recommended)` — Directors only at phase gate transitions (/gs-gate-check). Skips per-skill reviews. Balanced for solo devs and small teams.
...
and offer to handle it immediately using `ask_user_question`. Choose the first
branch that applies:
...
- "The most urgent fix is `systems-index.md` — [N] rows have parenthetical status
  values (e.g. `Needs Revision (see notes)`) that break /gs-gate-check,
  /gs-create-stories, and /gs-architecture-review right now. I can fix these in-place."
...
- "The most urgent fix is adding `## Status` to [N] ADR(s): [list filenames].
  Without it, /gs-story-readiness silently passes all ADR checks. Start with
...
- "The most urgent gap is missing Acceptance Criteria in [N] GDD(s):
  [list filenames]. Without them, /gs-create-stories can't generate stories.
...
- "No blocking gaps — this project is template-compatible. What next?"
  ...
  - "Run /gs-project-stage-detect for a broader health check"
  - "Done — I'll work through the plan at my own pace"

> **Adoption plan saved to `docs/adoption-plan-[date].md`.** Re-run `/gs-adopt` at any time to re-check remaining gaps as you complete them.
...
Usually executed by the `technical-director` role. Load `gs-roster` for the delegation protocol.

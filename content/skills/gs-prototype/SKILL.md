---
name: gs-prototype
description: Concept prototype — validate the core idea is worth designing before writing GDDs. Run right after /gs-brainstorm and /gs-setup-engine. Routes to HTML, Engine, or Paper path based on game type. Produces a throwaway build and a PROCEED/PIVOT/KILL verdict.
...
**Default use** — run right after `/gs-brainstorm` and `/gs-setup-engine`, before writing
...
before committing to Production, run `/gs-vertical-slice` instead.
...
Otherwise, use `ask_user_question` to confirm intent before proceeding:
...
[answer]." Suggest next step: `/gs-map-systems` or `/gs-design-system [mechanic]`.
...
Read `AGENTS.md` and `../../handbook/technical-preferences.md` for the engine and
language in use.
...
Assess which path best fits the hypothesis, then use `ask_user_question` with your
...
Read `../../templates/prototype-report.md` to get the report structure.
...
1. `/gs-design-review design/gdd/game-concept.md` — validate the concept doc against what the prototype revealed
2. `/gs-gate-check` — confirm readiness to advance to Systems Design
3. `/gs-art-bible` — define visual identity (optional but worth doing before GDDs)
4. `/gs-map-systems` — decompose the concept into all game systems
5. `/gs-design-system [mechanic]` — GDD for each MVP system; use prototype learnings
   in the Tuning Knobs and Formulas sections
6. `/gs-review-all-gdds` — cross-system consistency check
...
the revised hypothesis for the next prototype. When `/gs-prototype` is next run, check
...
- Run `/gs-prototype [revised-concept]` to test the adjusted direction
- Or `/gs-brainstorm [hint]` if the concept needs more fundamental rethinking
...
- Run `/gs-brainstorm open` or `/gs-brainstorm [new-hint]` to explore a different concept
...
2. **Choose path** — same ask_user_question widget as Phase 3 (HTML / Engine / Paper).
...
Usually executed by the `prototyper` role. Load `gs-roster` for the delegation protocol.

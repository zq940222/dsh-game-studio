---
name: gs-team-ui
description: "Orchestrate the UI team through the full UX pipeline: from UX spec authoring through visual design, implementation, review, and polish. Integrates with /gs-ux-design, /gs-ux-review, and studio UX templates."
...
- **engine UI specialist** — Validates UI implementation patterns against engine-specific best practices (read from `../../handbook/technical-preferences.md` Engine Specialists → UI Specialist)
...
- delegate to `ux-designer` (the child reads `roles/ux-designer.md` itself) — User flows, wireframes, accessibility, input handling
- delegate to `ui-programmer` (the child reads `roles/ui-programmer.md` itself) — UI framework, screens, widgets, data binding
- delegate to `art-director` (the child reads `roles/art-director.md` itself) — Visual style, layout polish, art bible consistency
- a subagent for `[UI engine specialist]` (the child reads its own brief under `roles/`) — Engine-specific UI pattern validation (e.g., unity-ui-specialist, ue-umg-specialist, godot-specialist)
- delegate to `accessibility-specialist` (the child reads `roles/accessibility-specialist.md` itself) — Accessibility compliance audit
...
Then use `ask_user_question` with options:
...
Before implementation begins, spawn the **engine UI specialist** (from `../../handbook/technical-preferences.md` Engine Specialists → UI Specialist) to review the UX spec and visual design spec for engine-specific implementation guidance:
...
delegated to sub-agents and sub-skills (`/gs-ux-design`, `ui-programmer`). Each enforces the

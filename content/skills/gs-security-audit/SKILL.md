...
Read `../../handbook/technical-preferences.md` to determine:
...
Spawn `security-engineer` via a subagent. Pass:
...
**Audited by**: security-engineer via /gs-security-audit
...
Run `/gs-security-audit` again after remediating any CRITICAL or HIGH findings.
...
After remediating findings, re-run: `/gs-security-audit quick` to confirm CRITICAL/HIGH items are resolved before running `/gs-gate-check release`.

If CRITICAL findings exist:
> "⛔ CRITICAL security findings must be resolved before any public release. Do not proceed to `/gs-launch-checklist` until these are addressed."

If no CRITICAL/HIGH findings:
> "✅ No blocking security findings. Report written to `production/security/`. Include this path when running `/gs-gate-check release`."
...
Usually executed by the `security-engineer` role. Load `gs-roster` for the delegation protocol.

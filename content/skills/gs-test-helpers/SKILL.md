...
**When to run:**
- After `/gs-test-setup` scaffolds the framework (first time)
...
**Modes:**
- `/gs-test-helpers [system-name]` — generate helpers for a specific system
  (e.g., `/gs-test-helpers combat`)
- `/gs-test-helpers all` — generate helpers for all systems with test files
- `/gs-test-helpers scaffold` — generate only the base helper library (no
...
Read `../../handbook/technical-preferences.md` and extract:
- `Engine:` value
- `Language:` value
- `Framework:` from the Testing section

If engine is not configured: "Engine not configured. Run `/gs-setup-engine` first."
...
## Generated by /gs-test-helpers combat on [date].
...
- Run `/gs-test-setup` if the test framework has not been scaffolded yet.
- Use `/gs-dev-story` to implement stories — helpers reduce boilerplate in new test files.
- Run `/gs-skill-test` to validate other skills that may need helper coverage.

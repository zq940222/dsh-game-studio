...
- After `/gs-team-qa` completes and new bugs have been filed
...
**Modes:**
- `/gs-bug-triage sprint` — triage against the current sprint; assign fixable bugs
  to the sprint backlog; defer the rest
- `/gs-bug-triage full` — full triage of all bugs regardless of sprint scope
- `/gs-bug-triage trend` — trend analysis only (no assignment); read-only report
...
Read `../../handbook/coding-standards.md` for severity/priority definitions if they
...
- If any S1 bugs are unassigned: "S1 bugs must be assigned before the sprint
  can be considered healthy. Run `/gs-sprint-status` to see current capacity."
- If regression bugs exist: "Regressions found — consider re-opening the
  affected stories in sprint tracking and running `/gs-smoke-check` to re-gate."

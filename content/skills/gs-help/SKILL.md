...
gap analysis, use `/gs-project-stage-detect`.

---

## Step 1: Read the Catalog

Read `../../pipeline/workflow-catalog.md`. This is the authoritative list of all
...
After reading the catalog, glob `../../skills/gs-*/SKILL.md` to get the full list
...
---
Approaching **[next phase]** gate → run `/gs-gate-check` when ready.
```
...
- If all required steps in the current phase are complete (or nearly complete),
  add: "You're close to the **[Current] → [Next]** gate. Run `/gs-gate-check` when ready."
...
---
Need more detail?
- `/gs-project-stage-detect` — full gap analysis with all missing artifacts listed
- `/gs-gate-check` — formal readiness check for your next phase
- `/gs-start` — re-orient from scratch
```

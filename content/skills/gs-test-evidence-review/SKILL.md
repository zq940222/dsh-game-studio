...
`/gs-smoke-check` verifies that test files **exist** and **pass**. This skill
...
**When to run:**
- Before QA hand-off sign-off (`/gs-team-qa` Phase 5)
...
**Modes:**
- `/gs-test-evidence-review [story-path]` — review a single story's evidence
- `/gs-test-evidence-review sprint` — review all stories in the current sprint
- `/gs-test-evidence-review [system-name]` — review all stories in an epic/system
...
- For BLOCKING items: "These must be resolved before `/gs-story-done` can mark the
  story Complete. Would you like to address any of them now?"
- For thin assertions: "Consider running `/gs-test-helpers [system]` to see

...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `ask_user_question`:
...
3. If no systems index exists, fail with:
   > "Usage: `/gs-design-system <system-name>` — e.g., `/gs-design-system movement`
   > Or to fill gaps in an existing GDD: `/gs-design-system retrofit design/gdd/[system-name].md`
   > No systems index found. Run `/gs-map-systems` first to map your systems and get the design order."
...
- **Game concept**: Read `design/gdd/game-concept.md` — fail if missing:
  > "No game concept found. Run `/gs-brainstorm` first."
- **Systems index**: Read `design/gdd/systems-index.md` — fail if missing:
  > "No systems index found. Run `/gs-map-systems` first to map your systems."
...
- Read `../../handbook/technical-preferences.md` to identify the engine and version
- If engine is configured, read `../../engines/[engine]/VERSION.md`
- Read `../../engines/[engine]/modules/[domain].md` if it exists
- Read `../../engines/[engine]/breaking-changes.md` for domain-relevant entries
...
> `/gs-setup-engine` before moving to architecture if you haven't already."
...
Use the template structure from `../../templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `ask_user_question` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `ask_user_question` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `ask_user_question`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `ask_user_question` with a multi-tab widget:
...
in `/gs-architecture-decision`, not the GDD. The GDD describes behavior; the ADR
...
**Framing question (ask BEFORE drafting)**: Use `ask_user_question`:
...
spawn `creative-director` via a subagent:
...
- Surface any disagreements between agents to the user via `ask_user_question`
...
**Agent delegation (MANDATORY)**: Spawn `qa-lead` via a subagent before finalising acceptance criteria. Provide: the completed GDD sections C, D, E, and ask them to validate that the criteria are independently testable and cover all core rules and formulas. Surface any gaps or untestable criteria to the user.
...
For required systems: **spawn `art-director` via a subagent** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
> "📌 **Asset Spec** — Visual/Audio requirements are defined. After the art bible is approved, run `/gs-asset-spec system:[system-name]` to produce per-asset visual descriptions, dimensions, and generation prompts from this section."
...
> (Pre-Production), run `/gs-ux-design` to create a UX spec for each screen or
...
Before finalizing the GDD, spawn `creative-director` via a subagent using gate **CD-GDD-ALIGN** (`../../handbook/director-gates.md`).
...
> `/gs-design-review design/gdd/[system-name].md`
>
> **Never run `/gs-design-review` in the same session as `/gs-design-system`.** The reviewing
...
**NEVER offer to run `/gs-design-review` inline.** Always direct the user to a fresh window.
...
- "What's next?"
  - Options:
    - "Run `/gs-consistency-check` — verify this GDD's values don't conflict with existing GDDs (recommended before designing the next system)"
    - "Design next system ([next-in-order])" — if undesigned systems remain
    - "Fix review findings" — if design-review flagged issues
    - "Stop here for this session"
    - "Run `/gs-gate-check`" — if enough MVP systems are designed
...
**When delegating via subagent tool**:
...
- The main session presents the agent's output to the user via `ask_user_question`
- The user decides; the main session writes to file
...
2. **ask_user_question** at every decision point (Explain -> Capture pattern):
...
> open a fresh session and run `/gs-design-system [system-name]` — it will
...
- Run `/gs-design-review design/gdd/[system-name].md` in a **fresh session** to validate the completed GDD independently
- Run `/gs-consistency-check` to verify this GDD's values don't conflict with other GDDs
- Run `/gs-map-systems next` to move to the next highest-priority undesigned system
- Run `/gs-gate-check pre-production` when all MVP GDDs are authored and reviewed

...
Read the relevant agent definition from `../../roles/` if a specific role is specified.
...
- Run `/gs-sprint-status` to show the new contributor current progress.
- Run `/gs-help` if the contributor needs guidance on what to work on next.

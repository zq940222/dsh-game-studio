...
**Argument:** `$ARGUMENTS` (blank = ask user via ask_user_question)
...
- **No argument** — use `ask_user_question`:
...
**Date**: [date]
**Generated by**: /gs-qa-plan
**Scope**: [N stories across [N systems]]
**Engine**: [engine name from ../../handbook/technical-preferences.md, or "Not configured"]
...
*Smoke tests are verified by the developer via `/gs-smoke-check`. Reference this
...
- [ ] Smoke check passes (run `/gs-smoke-check sprint` before QA hand-off)
- [ ] No regressions introduced
- [ ] Code reviewed (via `/gs-code-review` or documented peer review)
- [ ] Story file updated to `Status: Complete` (via `/gs-story-done`)
````
...
then ask two questions together using `ask_user_question`:
...
options:
  ...
  - "Also back-fill test case specs into each story file's ## QA Test Cases section (Recommended — enables /gs-dev-story and /gs-code-review traceability)"
```
...
- Once all sprint stories are implemented, run `/gs-smoke-check sprint` to gate QA hand-off — not yet, only after implementation is complete
...
- Use `ask_user_question` for scope selection when no argument is provided.

...
2. If a version is found: report "No version argument provided — inferred [version] from milestone data. Proceeding." Then confirm with `ask_user_question`: "Releasing [version]. Is this correct?"
3. If no version is discoverable: use `ask_user_question` to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
...
- delegate to `release-manager` (the child reads `roles/release-manager.md` itself) — Release branch, versioning, changelog, deployment
- delegate to `qa-lead` (the child reads `roles/qa-lead.md` itself) — Test sign-off, regression suite, release quality gate
- delegate to `devops-engineer` (the child reads `roles/devops-engineer.md` itself) — Build pipeline, artifacts, deployment automation
- delegate to `security-engineer` (the child reads `roles/security-engineer.md` itself) — Security audit for online/multiplayer/data features
- delegate to `analytics-engineer` (the child reads `roles/analytics-engineer.md` itself) — Telemetry event verification and dashboard readiness
- delegate to `community-manager` (the child reads `roles/community-manager.md` itself) — Patch notes and launch communication
- delegate to `producer` (the child reads `roles/producer.md` itself) — Go/no-go decision, stakeholder communication
- delegate to `network-programmer` (the child reads `roles/network-programmer.md` itself) — Netcode stability sign-off (invoke if game has multiplayer)
...
- Generate the release checklist using `/gs-release-checklist`
...
- Generate changelog using `/gs-changelog`
...
- Finalize patch notes using `/gs-patch-notes [version]`

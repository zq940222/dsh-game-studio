...
- If the file doesn't exist and this is a `new` sprint: use `ask_user_question`:
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `ask_user_question` to gather changes.
...
Before finalising the sprint plan, spawn `producer` via a subagent using gate **PR-SPRINT** (`../../handbook/director-gates.md`).
...
If [A]: close with "Sprint plan written. Run `/gs-qa-plan sprint` next — then begin implementation."
...
This file is read by `/gs-sprint-plan`, `/gs-story-readiness`, `/gs-story-done`, and other skills at startup.

...
to `/gs-create-stories [epic-slug]` or `/gs-quick-design` depending on the size of the gap.

### --summary mode

Print the Gap Table and Summary directly to conversation. Do not write a file.
End with: "Run `/gs-content-audit` without `--summary` to write the full report."
...
- If any system is `NOT STARTED` and MVP-tagged → "Run `/gs-design-system [name]` to
  add missing content counts to the GDD before implementation begins."
- If total gap is >50% → "Run `/gs-sprint-plan` to allocate content work across upcoming sprints."
- If backlog stories are needed → "Run `/gs-create-stories [epic-slug]` for each HIGH PRIORITY gap."
- If `--summary` was used → "Run `/gs-content-audit` (no flag) to write the full report to `docs/`."

Verdict: **COMPLETE** — content audit finished.

---

Usually executed by the `producer` role. Load `gs-roster` for the delegation protocol.

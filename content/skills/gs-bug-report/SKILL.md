...
2. **Run the related test** — if the bug's system has a test file in `tests/`, run it via the shell tool (`bash` on POSIX, `pwsh` on Windows) and report pass/fail.
...
If STILL PRESENT: reopen the bug, set Status back to Open, and suggest re-running `/gs-hotfix [BUG-ID]`.

---

## Phase 2D: Close Mode

Read `production/qa/bugs/[BUG-ID].md`. Confirm Status is `Verified Fixed` before closing. If status is anything else, stop: "Bug [ID] must be Verified Fixed before it can be closed. Run `/gs-bug-report verify [BUG-ID]` first."
...
After closing, check `production/qa/bug-triage-*.md` — if the bug appears in an open triage report, note: "Bug [ID] is referenced in the triage report. Run `/gs-bug-triage` to refresh the open bug count."
...
- Run `/gs-bug-triage` to prioritize alongside existing open bugs
- If S1 or S2: run `/gs-hotfix [BUG-ID]` for emergency fix workflow

**After fixing the bug (developer confirms fix is in):**
- Run `/gs-bug-report verify [BUG-ID]` — confirm the fix actually works before closing
- Never mark a bug closed without verification — a fix that doesn't verify is still Open

**After verify returns VERIFIED FIXED:**
- Run `/gs-bug-report close [BUG-ID]` — write the closure record and update status
- Run `/gs-bug-triage` to refresh the open bug count and remove it from the active list

...
whether the core idea is worth designing, run `/gs-prototype` (concept prototype) instead.
...
- Naming conventions from `../../handbook/technical-preferences.md`
...
Read `../../templates/vertical-slice-report.md` to get the report structure.
...
- `full` → spawn `creative-director` via a subagent using gate **CD-PLAYTEST**
...
Recommended next steps:
- `/gs-create-epics layer:foundation` — plan Foundation layer epics
- `/gs-create-epics layer:core` — plan Core layer epics
- `/gs-create-stories [epic-slug]` — break each epic into implementable stories
- `/gs-sprint-plan` — plan the first sprint using velocity data from the slice
- `/gs-gate-check pre-production` — formally advance the stage to Production

**Playtest note:** `/gs-gate-check` will look for documented playtest evidence.
...
differently. When `/gs-vertical-slice` is next run after a PIVOT, check the
`prototypes/` directory for a `PIVOT-NOTE.md` — use it to frame the new validation
question and inform scope decisions.

- Revise affected GDDs with `/gs-design-system [mechanic]`
- Address architecture issues via `/gs-architecture-decision`
- Then re-run `/gs-vertical-slice` to validate the revised direction
...
- Return to `/gs-brainstorm` with what you learned
- Or run `/gs-prototype [new-concept]` to test a new direction cheaply first

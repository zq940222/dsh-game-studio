...
Set once during `/gs-start`. Edit the file directly to change it at any time.
...
Examples:
```
/gs-brainstorm space horror           → uses global mode
/gs-brainstorm space horror --review full   → forces full mode this run
/gs-architecture-decision --review solo     → skips all gates this run
```
...
| `lean` | PHASE-GATEs only (`/gs-gate-check`) — per-skill gates skipped | **Default** — solo devs and small teams; directors review at milestones only |
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `ask_user_question` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
**Trigger**: Always at `/gs-gate-check` — spawn in parallel with TD-PHASE-GATE and PR-PHASE-GATE
...
**Trigger**: After `/gs-map-systems` Phase 3 dependency mapping is agreed but before
...
- Engine version and LLM knowledge cutoff (from `../engines/[engine]/VERSION.md`)
...
**Trigger**: Always at `/gs-gate-check` — spawn in parallel with CD-PHASE-GATE and PR-PHASE-GATE
...
**Trigger**: At milestone review (`/gs-milestone-review`), at mid-sprint retrospectives,
...
**Trigger**: Always at `/gs-gate-check` — spawn in parallel with CD-PHASE-GATE and TD-PHASE-GATE
...
- Game pillars and core fantasy
- Platform and performance constraints (from `../handbook/technical-preferences.md` if configured)
...
**Trigger**: Always at `/gs-gate-check` — spawn in parallel with CD-PHASE-GATE, TD-PHASE-GATE, and PR-PHASE-GATE
...
`/gs-story-readiness`, and `/gs-sprint-plan` during story selection
...
done, or at `/gs-gate-check` Production → Polish
